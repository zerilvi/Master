...
public class GridPoint
{
    ...
    private float[] minDist; // every point to isosurface minDist;
...
public static class MarchingCube
{
    ...
    public static void calDist(Mesh mesh)
    {
        ...
        for (int z = 0; z < grd.GetLength(2) - 1; z++)
        {
            for (int y = 0; y < grd.GetLength(1) - 1; y++)
            {
                for (int x = 0; x < grd.GetLength(0) - 1; x++)
                {
                    ...
                    {
                        float dist = Vector3.Distance(grd[x, y, z].Position, mesh.vertices[i]);
                        i++;
                    }
